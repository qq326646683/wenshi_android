...
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public BottomSheetBehavior(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        if (hasBackgroundTint) {
            ColorStateList bottomSheetColor = getColorStateList(
    ...
    public boolean onInterceptTouchEvent(
        ...
        // 触碰箭头区域 可以拖动
        if (!(event.getX() == 0 && event.getY() == 0 && event.getAction() == MotionEvent.ACTION_CANCEL)) {
            if (state == STATE_COLLAPSED) {
                draggable = inRangeOfView(((ViewGroup) child).getChildAt(0), event);
            }
            if (state == STATE_EXPANDED) {
                // STATE_EXPANDED 情况下  recyclerview 滑动到最顶部 需要处理联动逻辑
                if (inRangeOfView(((ViewGroup) child).getChildAt(1), event)) {
                    draggable = true;
                }
            }
        }
        ...
        View scroll = nestedScrollingChildRef != null ? nestedScrollingChildRef.get() : null;
        boolean result = action == MotionEvent.ACTION_MOVE
                && scroll != null
                && !ignoreEvents
    ...
    private void settleToStatePendingLayout(@BottomSheetBehavior.State int state) {
    ...
    private void updateDrawableForTargetState(@BottomSheetBehavior.State int state) {
        if (state == STATE_SETTLING) {
    ...
    private void setSystemGestureInsets(@NonNull View child) {
        if (VERSION.SDK_INT >= 29 && !isGestureInsetBottomIgnored() && !peekHeightAuto) {
    ...
    void startSettlingAnimation(View child, int state, int top, boolean settleFromViewDragHelper) {
        ...
        if (startedSettling) {
            ...
            if (settleRunnable == null) {
                ...
                settleRunnable = new BottomSheetBehavior.SettleRunnable(child, state);
            }
